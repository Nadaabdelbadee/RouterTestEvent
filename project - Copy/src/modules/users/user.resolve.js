...
export const logOut = async (parent, args) => {
  const { email } = args;
...
export const forgetPassword = async (parent, args) => {
  const { email } = args;
  const user = await userModel.findOne({ email, isDeleted: false });
  if (!user) {
    throw new Error("user not exist");
...
export const confirmOtp = async (parent, args) => {
...
export const resetPassword = async (parent, args) => {
